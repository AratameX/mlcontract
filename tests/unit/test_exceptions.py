...
from schemapact import ContractDefinitionError, IntegrationError, SchemaPactError
from schemapact.exceptions import REGISTRY, ErrorCode, missing_dependency


class TestHierarchy:
    @pytest.mark.parametrize("cls", [ContractDefinitionError, IntegrationError])
    def test_everything_descends_from_the_base(self, cls):
        """Callers must be able to catch SchemaPactError and get everything."""
        assert issubclass(cls, SchemaPactError)

    def test_base_is_an_exception(self):
        assert issubclass(SchemaPactError, Exception)
...
class TestErrorPayload:
    def test_message_carries_the_code(self):
        error = SchemaPactError("something broke", code=ErrorCode("SPX000", "test"))
        assert str(error) == "[SPX000] something broke"

    def test_context_is_structured(self):
        """Details must be inspectable without parsing the message string."""
        error = SchemaPactError(
            "bad value", code=ErrorCode("SPX000", "test"), field="age", expected=18
        )
        assert error.context == {"field": "age", "expected": 18}

    def test_message_is_available_without_the_code_prefix(self):
        error = SchemaPactError("plain", code=ErrorCode("SPX000", "test"))
...
class TestRegistry:
    ...
    def test_error_code_str_is_the_bare_code(self):
        assert str(ErrorCode("SPX001", "x")) == "SPX001"
...
class TestMissingDependency:
    def test_names_the_exact_install_command(self):
        ...
        assert 'pip install "schemapact[yaml]"' in str(error)
        assert error.code.code == "SPX901"
